            ...
            "findItemsAdvanced",
            {
                ...
                "itemFilter": [
                    {"name": "LocalPickupOnly", "value": True},
                    {
                        "name": "MaxDistance",
                        "value": "5",
                    },  # No value set for location 2, so use default value
                    {"name": "Condition", "value": "Used"},
                    {"name": "ListingType", "value": "Auction"},
                ],
            ...
            "findItemsAdvanced",
            {
                ...
                "itemFilter": [
                    {"name": "LocalPickupOnly", "value": False},
                    {
                        "name": "MaxDistance",
                        "value": "20",
                    },  # `MockKwSearch` sets custom value for location 1
                    {"name": "Condition", "value": "Used"},
                    {"name": "ListingType", "value": "Auction"},
            ...
            "findItemsAdvanced",
            {
                ...
                "itemFilter": [
                    {"name": "LocalPickupOnly", "value": False},
                    {
                        "name": "MaxDistance",
                        "value": "5",
                    },  # No custom value set for location 2, so use default value
                    {"name": "Condition", "value": "Used"},
                    {"name": "ListingType", "value": "Auction"},
                    {"name": "MaxPrice", "value": 25},
                ],
            ...
            "findItemsAdvanced",
            {
                ...
                "itemFilter": [
                    {"name": "LocalPickupOnly", "value": False},
                    {
                        "name": "MaxDistance",
                        "value": "10",
                    },  # MockKwSearch` sets custom value for location 3
                    {"name": "Condition", "value": "Used"},
                    {"name": "ListingType", "value": "Auction"},
                    {"name": "MaxPrice", "value": 25},
                ],
                ...
                "itemFilter": [
                    {
                        "name": "MaxDistance",
                        "value": "10",
                    },  # `mock_kw_search` overrides custom value for location 1
                ],
                "buyerPostalCode": "AB1 2CD",
            },
        ),
                ...
                "itemFilter": [
                    {
                        "name": "MaxDistance",
                        "value": "10",
                    },  # mock_kw_search` uses default search radius anyway
                ],
                "buyerPostalCode": "EF3 5GH",
            },
        ),
                ...
                "itemFilter": [
                    {
                        "name": "MaxDistance",
                        "value": "10",
                    },  # mock_kw_search` overrides custom value for location 3
                ],
                "buyerPostalCode": "IJ6 7KL",
            },
        ),
...
def test_search_filters_merged_keys(child_search_filters):
    ...
    parent_kw_search = (
        KeywordSearch()
    )  # KeywordSearch defines `search_filters = {'LocalPickupOnly': True}`
    child_kw_search = ChildKeywordSearch()

    result = child_kw_search.search_filters

    assert (
        child_search_filters.items() <= result.items()
    )  # All child search filters must be present in the result
    assert (
        "LocalPickupOnly" in result.keys()
    )  # Parent `search_filters` key must be inherited (regardless of child `search_filters` defintion)
    assert parent_kw_search.search_filters == {
        "LocalPickupOnly": True,
        "MaxDistance": "5",
    }  # Original keys/values must remain in parent
...
def test_search_filters_unmerged_keys(child_search_filters):
    ...
    assert (
        result.items() == child_search_filters.items()
    )  # All child search filters must be present in the result
    assert parent_kw_search.search_filters == {
        "LocalPickupOnly": True,
        "MaxDistance": "5",
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {
            "location 1": conftest.generate_mock_response(
                conftest.MOCK_SEARCH_RESULT_THREE_ITEMS
            ).reply.searchResult,
            "location 2": conftest.generate_mock_response(
                conftest.MOCK_SEARCH_RESULT_THREE_ITEMS
            ).reply.searchResult,
            "location 3": conftest.generate_mock_response(
                conftest.MOCK_SEARCH_RESULT_THREE_ITEMS
            ).reply.searchResult,
        },
    ],
...
def test_results_no_cache_calls_find_items(mocker, mock_kw_search, mock_cached_results):
    ...
    assert spy.called is not bool(
        mock_cached_results
    )  # Method must be called when there is no cached data, and vice versa
    assert result
    ...
    assert all(
        [
            isinstance(location_result, ebaysdk.response.ResponseDataObject)
            for location_result in result.values()
        ]
    )
...
def test_results_removes_duplicates_items_found(mock_kw_search):
    ...
    mock_kw_search.cached_results = {
        "location 1": conftest.generate_mock_response(
            conftest.MOCK_SEARCH_RESULT_ZERO_ITEMS
        ).reply.searchResult,
        "location 2": conftest.generate_mock_response(
            conftest.MOCK_SEARCH_RESULT_THREE_ITEMS
        ).reply.searchResult,
    ...
    assert (
        len(getattr(result["location 1"], "item", list())) == 0
    )  # location 1 was mocked to contain zero items
    assert len(getattr(result["location 2"], "item", list())) == 3
    assert (
        len(getattr(result["location 3"], "item", list())) == 0
    )  # location 3 was mocked to contain same items as location 2, so remove all
