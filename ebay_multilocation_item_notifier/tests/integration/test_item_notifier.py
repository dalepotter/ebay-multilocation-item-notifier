...
kw_container = KeywordSearchContainer(Bike())
...
def test_main(capsys, mocker, caplog):
    ...
    stdout_lines = [
        x for x in filter(len, stdout.split("\n"))
    ]  # Split by new line, but exclude empty lines

    results_per_location = [
        int(re.search(r"Results for this location: (\d+)", line).group(1)) > 0
    ...
    assert (
        len(stdout_lines) == 3
    )  # There much be an output for each search item/location permutation
    assert all(
        ["(bike, bicycle)" in x for x in stdout_lines]
    )  # The keyword must be outputeed in each line
    assert all(
        [
            int(re.search(r"Results for this location: (\d+)", line).group(1)) > 0
            for line in stdout_lines
    ...
    assert (
        email_html_call["subject"] == "[ebay-multilocation-item-notifier] Item summary"
    )
