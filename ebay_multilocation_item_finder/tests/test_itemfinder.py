...
def generate_mock_response(search_results):
    """Return a mock ebaysdk.response.Response object that can be used to patch the output of itemfinder.make_ebay_request."""
...
def mock_response_three_items():
    # Define data for a mock ebaysdk.response.ResponseDataObject search containing 3 items
    mock_result = {
      'item': [
          {
            'itemId': '143183113840',
    ...
    return generate_mock_response(mock_result)


def test_get_results_dict_return_format(mocker, mock_response_three_items):
    """`get_results_dict` must return a dictionary."""
...
def test_get_results_dict_items_parsed(mocker, mock_response_three_items):
    mocker.patch.object(ebaysdk.finding.Connection, 'execute')
    ebaysdk.finding.Connection.execute.return_value = mock_response_three_items

    result = get_results_dict(['search keyword 1'], [('location 1', 'PL4 6AB', 20)])
